...
## Where things live
- `src/App.tsx` — Main state machine: file upload, phase orchestration, parallel analysis, download modal
...
- `src/types.ts` — Shared types: Document, Batch, Decision, CorrectedFile, AppConfig, JSON phase types
...
- **Context Isolation**: F1 gets all files; F2 gets only F1 JSON; F3 gets only F2 JSON; F4 gets F2+F3 JSON **plus the original files from the selected batches** — required so the AI can produce complete corrected files with surgical edits, not just snippets.
...
- **Telemetry panel**: full header row clickable → expanded modal; streaming verbose shown at TOP of panel when active; provider/model/connection status row; F1–F4 tabs; F4 tab activates when any batch has executionMd (not just when correctedFiles.length > 0)
...
- Download modal: ZIP COMPLETO, ZIP só modificados, individual files
...
npm run dev
```
App served on port 5173 (or auto-assigned Replit port).
...
## Gotchas
- F4 JSON parsing is fragile when AI embeds file contents with unescaped newlines/quotes — `extractJSON` has 4 levels of robustness but may still fail on very large responses. The error now surfaces via `setError()` with an actionable message.
- `DECISION_OPTION_COLORS` is exported from Registry.tsx causing Vite HMR "incompatible export" warnings — this is cosmetic and does not affect runtime.
