...
import { Zap, Info, RefreshCcw, Settings, ChevronDown, Check, Globe, Layout, ShieldCheck, HelpCircle, X } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({ 
  ...
  const googleModels: { id: ModelType; label: string; desc: string }[] = [
    { id: 'gemini-3.1-pro-preview', label: 'Gemini 3.1 Pro', desc: 'Raciocínio avançado & Lógica complexa' },
    { id: 'gemini-3-flash-preview', label: 'Gemini 3 Flash', desc: 'Equilíbrio ideal entre velocidade e inteligência' },
    { id: 'gemini-3.1-flash-lite-preview', label: 'Gemini 3.1 Lite', desc: 'Extremamente rápido & Baixa latência' },
    { id: 'gemini-2.0-flash-exp', label: 'Gemini 2.0 Flash Exp', desc: 'Experimental de próxima geração' },
    { id: 'gemini-flash-latest', label: 'Flash Latest', desc: 'Sempre a última versão estável' }
  ];
  ...
  return (
    <header className="h-16 border-b border-white/5 bg-white/5 flex items-center justify-between px-4 md:px-6 z-50 shrink-0 relative">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="hidden xs:flex text-[10px] text-slate-500 font-mono items-center gap-1">
            <span className="hidden sm:inline">CORE LAYER: <span className="text-slate-400">DEEP_SCAN_V4.1.0</span></span>
      ...
      <div className="flex items-center gap-4">
        <div className="hidden md:flex items-center gap-4 px-4 py-1.5 bg-white/5 rounded-lg border border-white/5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-mono text-slate-400 uppercase tracking-widest">Status: <span className={isLoading ? 'text-orange-400' : 'text-emerald-400'}>{isLoading ? 'Processando' : 'Online'}</span></span>
          </div>
          <div className="w-px h-3 bg-white/10" />
          <div className="flex items-center gap-2 text-cyan-400">
             <Layout className="w-3 h-3" />
             <span className="text-[10px] font-mono uppercase tracking-widest">{parsedFilesCount}/{fileCount} Analisados</span>
             <span className="text-white/20">|</span>
             <span className="text-[10px] font-mono uppercase tracking-widest">{refactoredCount} Fixes</span>
          </div>
        </div>
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
              ...
              >
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="px-2 py-0.5 bg-cyan-500/10 border border-cyan-500/20 rounded text-[8px] font-mono text-cyan-400">v4.1</div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {providers.map(p => {
                        ...
                        return (
                          <button
                            key={p.id}
                            onClick={() => onConfigChange({ ...config, provider: p.id, model: p.id === 'google' ? 'gemini-1.5-pro' : config.model })}
                  ...
                  <div>
                    {config.provider === 'google' ? (
                      ...
                      <div className="space-y-3">
                        <div>
                          <label className="text-[9px] uppercase font-black text-slate-500 mb-2 block tracking-widest">Model ID (ex: claude-3)</label>
                          <input 
                            type="text"
